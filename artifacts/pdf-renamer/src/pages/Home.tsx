...
export default function Home() {
  ...
  async function aiResolveUnrecognized() {
    ...
    try {
      ...
      const scanJobId = localStorage.getItem("scanJobId");

      const res = await fetch(`${BASE}/api/dropbox/ai-resolve/start`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ files: payload, scanJobId }),
      });
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {done && readyCount > 0 && (
              <button
                onClick={() => {
                  const jobId = localStorage.getItem("scanJobId");
                  if (jobId) window.open(`${BASE}/api/dropbox/export-csv?jobId=${jobId}`, "_blank");
                }}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
